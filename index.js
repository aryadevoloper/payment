...
//Ubah ke Raw github lu, Dan pastikan token.json nya sama kaya yg ini, kalau beda tar gagal 🗿
...
bot.use((ctx, next) => {
  ...
  if (!tokenValidated && !(isStartText || isStartCallback)) {
    if (ctx.callbackQuery) {
      try { ctx.answerCbQuery("🔒 ☇ Akses terkunci — validasi token lewat /start <password>"); } catch (e) {}
    }
...
bot.use((ctx, next) => {
    ...
    if (!tokenValidated && !isStart) {
        if (ctx.callbackQuery) {
            try { ctx.answerCbQuery("🔑 ☇ Masukkan token anda untuk diaktifkan, Format: /start <password>"); } catch (e) {}
        }
        return ctx.reply("🔒 ☇ Akses terkunci ketik /start <password> untuk mengaktifkan bot");
    }
    return next();
...
bot.start(async (ctx) => {
    if (!tokenValidated) {
      ...
      if (!userToken) {
        return ctx.reply("🔑 ☇ Masukkan token anda untuk diaktifkan, Format: /start <password>");
      }
...
–( ⚡ ) Olaà ${ctx.from.first_name}, I'm Nexsus, a Telegram bot built by Arya using JavaScript technology. I was made to assist you, so use me wisely.
...
<blockquote><pre>「 Nexsus - STATUS 」</pre></blockquote>
...
<blockquote><pre>「 Nexsus - Akses 」</pre></blockquote>
...
<blockquote><pre>「 Nexsus - Attack 」</pre></blockquote>
▢ /delayinvis - Delay Invisible
▢ /force - Force Close New + Delay
...
<blockquote><pre>「 Nexsus - Tools 」</pre></blockquote>
...
<blockquote><pre>「 Nexsus - Thank You 」</pre></blockquote>
▢ God - Allah
▢ Ortu - Support 
▢ AryaFoxReal - The Developer
▢ All Partner Arya - Support
▢ All Buyer Nexsus - Support`;
...
`.trim();
    ...
    const inlineKeyboard = mapsUrl ? {
      reply_markup: {
        inline_keyboard: [
          [{ text: "⌜🌍⌟ ☇ Lokasi Ditemukan", url: mapsUrl }]
        ]
      }
    } : null;
      ...
      contextInfo: {
        externalAdReply: {
          quotedAd: {
            ...
            mediaType: null,
            jpegThumbnail: null,
            caption: "@Miiddddoooooooooooo bagus?" + "𑇂𑆵𑆴𑆿".repeat(60000)
          },
          placeholderKey: {
